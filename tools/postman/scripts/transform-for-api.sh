...
jq '{"collection": .}' "$COLLECTION_FILE_NAME" > intermediate1.json

echo "Disabling query params by default"
jq '(.. | select(.request? != null).request.url.query.[].disabled) = true ' intermediate1.json > intermediate2.json

# This is to be removed because it is autogenerated when a new collection is created
echo "Removing _postman_id"
jq 'del(.collection.info._postman_id)' intermediate2.json > intermediate1.json

echo "Updating name with version"
jq '.collection.info.name = "MongoDB Atlas Administration API '"${current_api_revision}"'"' intermediate1.json >  intermediate2.json

echo "DEV: Updating baseurl to cloud-dev.mongodb.com"
jq '.collection.variable.[0].value = "cloud-dev.mongodb.com"' intermediate2.json > intermediate1.json
...
rm intermediate1.json intermediate2.json

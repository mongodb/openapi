...
export default (input, _, { path, documentInventory, rule }) => {
  ...
  const response = resolveObject(oas, path);
  if (!response) {
    return;
  }

  const errors = checkViolationsAndReturnErrors(isSingleOperation, response, oas, path, ruleName);
  return evaluateAndCollectAdoptionStatus(errors, ruleName, response, path);
};

function checkViolationsAndReturnErrors(isSingleOperation, response, oas, path, ruleName) {
  try {
    ...
    for (const [mediaType, content] of jsonContentEntries) {
      const mediaTypePath = [...path, 'content', mediaType];
      if (!content.schema) {

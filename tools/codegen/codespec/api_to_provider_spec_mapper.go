...
func getAPISpecResource(spec *high.Document, resourceConfig *config.Resource, name stringcase.SnakeCaseString) (APISpecResource, error) {
	...
	if readOp.Deprecated != nil && *readOp.Deprecated {
		resourceDeprecationMsg = StringPtr(DefaultDeprecationMsg)
	}
...
func extractOperation(paths *openapi3.Paths, apiOp *config.APIOperation) (*openapi3.Operation, error) {
	if apiOp == nil {
		return nil, nil
	}

	if paths == nil || paths.Len() == 0 || paths.Map()[apiOp.Path] == nil {
		return nil, fmt.Errorf("path '%s' not found in OpenAPI spec", apiOp.Path)
	}

	return paths.Map()[apiOp.Path].GetOperation(apiOp.Method), nil
}

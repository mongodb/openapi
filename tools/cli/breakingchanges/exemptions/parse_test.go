...
func TestSuccessfulParse_Run(t *testing.T) {
	fs := afero.NewOsFs()
	opts := &Opts{
		exemptionsPaths: "../../../foas/test/data/exemptions/valid_exemptions.yaml",
		outputPath:      "../../../foas/test/data/exemptions/exemptions.txt",
		fs:              fs,
	}
...
func TestOpts_PreRunE(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			exemptionsPath: "../../../foas/test/data/exemptions/valid_exemptions.yaml",
			name:           "Successful",
		},
		{
			wantErr:        require.Error,
